...
from dunkyimages.learning_rate_schedulers import StepDecay
from dunkyimages.learning_rate_schedulers import PolynomialDecay
from dunkyimages.resnet import ResNet
...
# then initializige the list of callbacks and learning rate scheduler to be used
epochs = args["epochs"]
callbacks = []
schedule = None
...
# loading the training and testing data, then scaling it into range [0, 1]
...
# initializing our optimizer and model, then compile it
...
# if the learning rate schedule is not empty, then save the learning rate plot
if schedule is not None:
	schedule.plot(N)
